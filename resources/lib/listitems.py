...
class listitems(object):
	def __init__(self,config=None,media_type=None):
		...
		self.json_decode_keys = set([x for x in self.string_to_list_keys if x not in self.config.listitem.get('non_string_to_list_keys')])  #These keys are stored in the db as json serializable.  Consider just changing all of them to meet this
	...
	def from_factory(self,row):
		if isinstance(row.get('localization'),int): #If the localized string ID was found, use it
			li = xbmcgui.ListItem(label=self.config.addon.get('addon_handle').getLocalizedString(row.get('localization')),offscreen=True)
		else: #Use the default (english) label
			li = xbmcgui.ListItem(label=row.get('label'),offscreen=True)
